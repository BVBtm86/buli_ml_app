...
def pca_application(data_app, data_map, all_features, main_filter, app_filter, feature_filter, feature_code, data_file):
    ...
    pca_sample = data_app[(data_app[feature_filter] == feature_code)].shape[0]
    if data_file == "All Statistics" and pca_sample <= 60:
        run_pca = False
    else:
        run_pca = True
    ...
    if len(analysis_stats) > 1:
        if run_pca:
            ...
            available_features = [var for var in data_map["Statistics"].unique() if
                                  var != 'Team' and var != main_filter]
