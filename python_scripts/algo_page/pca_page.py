...
def pca_application(data_app, data_map, all_features, main_filter, app_filter, feature_filter, feature_code):
    ...
    if len(analysis_stats) > 1:
        ...
        if pca_analysis == 'Table':
            ...
            with pca_col:
                st.plotly_chart(fig_corr,
                                ...
                                use_container_width=True)
        with feature_col:
            st.markdown(f"The recommended no of final <b><font color=#6600cc>Principal Components</font></b> is "
                        f"<b><font color=#6600cc>{pca_optimum}</font></b>", unsafe_allow_html=True)
        ...
        # ##### PCA Eigenvalues
        with optimum_col:
            st.table(pca_corr.style.background_gradient(cmap='PRGn', vmin=-1, vmax=1).set_table_styles(
                [{'selector': 'th',
                  'props': [('background-color', '#aeaec5'), ('color', '#ffffff')]
                  }]).format(formatter="{:.3f}"))

            st.markdown(f"If we keep <b><font color=#6600cc>{no_final_components}</font></b> Components we keep "
                        f"<b><font color=#6600cc>{final_variance:.2%}</font></b> of the original variance",
        ...
        _, download_col_1, download_col_2, download_col_3, _ = st.columns([1, 3, 3, 3, 1])
        with download_col_1:
